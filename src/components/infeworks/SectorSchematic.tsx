...
function Unit({ x, y, w, h, l, sub }: Box) {
  return (
    <g>
      ...
      <line x1={x} y1={y + 16} x2={x + w} y2={y + 16} stroke={LINE} strokeWidth="0.6" opacity="0.5" />
...
function Tag({ x, y, text, colour = COPPER }: { x: number; y: number; text: string; colour?: string }) {
  return (
...
function Pump({ x, y }: { x: number; y: number }) {
  return (
    <g stroke={LINE} strokeWidth="1.2" fill="none">
      ...
      <polygon points={`${x - 5},${y - 7} ${x - 5},${y + 7} ${x + 8},${y}`} fill={ACCENT} opacity="0.5" stroke="none" />
    </g>
  );
}
...
function Frame({ children }: { children: React.ReactNode }) {
  return (
    <svg viewBox="0 0 900 300" className="h-full w-full" fontFamily="var(--font-mono, ui-monospace)">
...
function WaterTreatment() {
  return (
    <Frame>
      ...
      {[118, 140, 162].map((y) => (
        <line key={y} x1={544} y1={y} x2={660} y2={y} stroke={ACCENT} strokeWidth="3" opacity="0.35" />
      ))}
...
function Wastewater() {
  return (
    <Frame>
      ...
      {/* RAS / sludge */}
      <path d="M670 180 L670 236 L500 236" fill="none" stroke={COPPER} strokeWidth="1.2" strokeDasharray="4 4" />
      ...
      <Tag x={24} y={268} text="Discharge compliance verified by laboratory sampling" colour={ACCENT} />
    </Frame>
  );
}
    ...
    <Frame>
      ...
      {[80, 150, 220].map((y, i) => (
        <g key={y}>
          ...
          <polygon points={`${360},${y - 8} ${360},${y + 8} ${374},${y}`} fill="none" stroke={LINE} strokeWidth="1.1" />
          <line x1={374} y1={y} x2={430} y2={y} stroke={ACCENT} strokeWidth="1.3" />
          <Tag x={252} y={y - 20} text={i === 2 ? "Standby" : `Duty 0${i + 1}`} colour={i === 2 ? COPPER : ACCENT} />
        </g>
      ))}
      {/* discharge header */}
      ...
      <Tag x={24} y={268} text="Duty point set from measured system curve · witnessed performance test" colour={ACCENT} />
    </Frame>
  );
}

/* ── irrigation: head station ────────────────────────────────────────────── */
function Irrigation() {
  return (
    <Frame>
      <Tag x={24} y={30} text="Irrigation Head Station" colour={ACCENT} />
      <Unit x={24} y={120} w={112} h={64} l="Source" sub="Canal / well" />
      <Flow x1={136} y1={152} x2={168} y2={152} />
      <Pump x={184} y={152} />
      <Flow x1={200} y1={152} x2={232} y2={152} />
      <Unit x={232} y={120} w={124} h={64} l="Sand filter" sub="Back-washable" />
      <Flow x1={356} y1={152} x2={388} y2={152} />
      <Unit x={388} y={120} w={124} h={64} l="Screen filter" sub="Disc / mesh" />
      <Flow x1={512} y1={152} x2={544} y2={152} />
      <Unit x={544} y={104} w={140} h={96} l="Fertigation" sub="Venturi dosing" />
      {[124, 144, 164].map((y) => (
        <line key={y} x1={556} y1={y + 22} x2={672} y2={y + 22} stroke={COPPER} strokeWidth="1" opacity="0.5" />
      ))}
      <Unit x={544} y={30} w={140} h={44} l="Nutrient tanks" />
      <Flow x1={614} y1={74} x2={614} y2={98} dashed />
      {/* zoning manifold */}
      <line x1={684} y1={152} x2={730} y2={152} stroke={ACCENT} strokeWidth="1.6" />
      <line x1={730} y1={72} x2={730} y2={236} stroke={ACCENT} strokeWidth="2.2" />
      {[86, 124, 162, 220].map((y, i) => (
        <g key={y}>
          <line x1={730} y1={y} x2={856} y2={y} stroke={ACCENT} strokeWidth="1.1" opacity="0.8" />
          <circle cx={868} cy={y} r="5" fill="none" stroke={LINE} strokeWidth="1.1" />
          <Tag x={744} y={y - 8} text={`Zone 0${i + 1}`} colour={ACCENT} />
        </g>
      ))}
      <Tag x={24} y={268} text="Pressure zoning and scheduling by field block" colour={ACCENT} />
    </Frame>
  );
}
...
function ElectricalControl() {
  return (
    <Frame>
      ...
      {/* supervisory */}
      <Unit x={480} y={30} w={396} h={52} l="Operations dashboard" sub="Output logging · alarm escalation · monthly report" />
...
const MAP: Record<SectorSlug, () => React.ReactElement> = {
  ...
  "electrical-control": ElectricalControl,
};
